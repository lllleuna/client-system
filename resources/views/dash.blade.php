...
--}}
    ...
    <div class="container mx-auto px-6">
        ...
        <div x-data="{
            currentSlide: 0,
            {{-- announcements: {{ json_encode($announcements ?? [ --}}['title' => 'Welcome to OTC Portal', 'content' => 'Manage your cooperative efficiently', 'color' => 'bg-blue-500'],
            ['title' => 'New Guidelines Released', 'content' => 'Check the latest updates', 'color' => 'bg-green-500'],
            ['title' => 'Upcoming Training', 'content' => 'Register now for next month'
                s session ', '
                color ' => '
                bg - purple - 500 ']
            ])
                        ...
                        @foreach ($actions as $index => $action)
                            ...
                            @else
                                <a href="{{ route($action['route']) }}"
                                    ...
                                    <span
                                        class="font-medium text-{{ $action['color'] }}-700">{{ $action['text'] }}</span>
                                    <svg class="w-5 h-5 text-{{ $action['color'] }}-600" fill="none"
