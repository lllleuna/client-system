                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6">
                    ...
                    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 space-y-4 lg:space-y-0">
                        ...
                        <div class="flex flex-col sm:flex-row w-full lg:w-auto space-y-3 sm:space-y-0 sm:space-x-3">
                            <a href="{{ route('addCoopUnitIndex') }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-200">
                            ...
                            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        @foreach ($coopUnits as $coopunit)
                                        <tr class="hover:bg-gray-50" id="unit-{{ $coopunit->id }}">
                                            <td class="px-3 sm:px-6 py-4">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">{{ $coopunit->type}}</span>
                                            </td>
                                            <td class="hidden sm:table-cell px-3 sm:px-6 py-4 text-sm text-gray-500">{{ $coopunit->plate_no }}</td>
                                            <td class="hidden md:table-cell px-3 sm:px-6 py-4 text-sm text-gray-500">{{ $coopunit->ltfrb_case_no }}</td>
                                            <td class="hidden lg:table-cell px-3 sm:px-6 py-4 text-sm text-gray-500">{{ $coopunit->date_of_expiry }}</td>
                                            <td class="py-3 px-4 border text-center">
                                                <div class="flex justify-center space-x-2">
                                                    <a href="{{ route('editCoopUnit', $coopunit->id) }}" class="text-blue-600 hover:text-blue-800">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                        </svg>
                                                    </a>
                                                    <a href="javascript:void(0);" onclick="confirmDelete({{ $coopunit->id }}, '{{ $coopunit->plate_no }}')" class="text-red-600 hover:text-red-800">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    function confirmDelete(id, plate_no) {
        if (confirm(`Are you sure you want to delete ${plate_no} ?`)) {
            fetch(`/myinformation/coopownedunit/${id}`, {
                method: 'DELETE',
                headers: {
