                        ...
                        <div class="mt-6">
                            ...
                            <div class="bg-blue-50 p-4 rounded-lg mb-6">
                                        ...
                                        <tbody>
                                            @foreach ($coopGrants as $grant)
                                                <tr id="grant-{{ $grant->id }}">
                                                    <td class="py-3 px-4 border text-left">{{ $grant->date_acquired }}</td>
                                                    <td class="py-3 px-4 border text-left">₱{{ $grant->amount }}</td>
                                                    <td class="py-3 px-4 border text-left">{{ $grant->source }}</td>
                                                    <td class="py-3 px-4 border text-left">

                                                        <p class="text-sm text-gray-600 mt-1">{{ $grant->status_remarks }}
                                                        </p>
                                                    </td>
                                                    <td class="py-3 px-4 border text-center">
                                                        <div class="flex justify-center space-x-2">
                                                            <a href="{{ route('editGrant', $grant->id) }}"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                                    @foreach ($grantsPerYear as $grantYear)
                                        <div class="bg-white p-4 rounded-lg shadow-sm mb-4">
                                            <h4 class="text-md font-medium text-gray-600 mb-2">
                                                Total Received for {{ $grantYear->year }}
                                            </h4>
                                            <p class="text-2xl font-bold text-green-600">
                                                ₱{{ number_format($grantYear->total_amount, 2) }}
                                            </p>
                                            <p class="text-sm text-gray-500 mt-1">
                                                {{ $grantYear->total_grants }} grants/donations
                                            </p>
                                        </div>
                                    @endforeach
    ...
    <script>
        function confirmDelete(id, source) {
            if (confirm(`Are you sure you want to delete ${source}?`)) {
                fetch(`/myinformation/editgrants/${id}`, {
                        method: 'DELETE',
                        headers: {
                    ...
                    .then(data => {
                        alert(data.message);
                        let row = document.getElementById(`grant-${id}`);
                        if (row) {
